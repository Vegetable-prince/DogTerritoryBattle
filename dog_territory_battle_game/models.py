...
class Game(TimeStampedModel):
    player1 = models.ForeignKey(
        Player, related_name="player1_games", on_delete=models.CASCADE
    )
    player2 = models.ForeignKey(
        Player, related_name="player2_games", on_delete=models.CASCADE
    )
    current_turn = models.ForeignKey(
        Player, related_name="current_turn_games", on_delete=models.CASCADE
    )
...
class Dog(TimeStampedModel):
    ...
    def __str__(self):
        return (
            f"{self.dog_type.name} at ({self.x_position}, {self.y_position})"
            if not self.is_in_hand
            else f"{self.dog_type.name} in hand"
        )
